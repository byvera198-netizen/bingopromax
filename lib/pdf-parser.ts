...
function sortSpatialItems<T extends { x: number; y: number }>(items: T[]): T[] {
  const itemsCopy = [...items].sort((a, b) => b.y - a.y);
  const rows: T[][] = [];
  for (const item of itemsCopy) {
    const row = rows.find((r) => Math.abs(r[0].y - item.y) <= 35);
    if (row) {
      row.push(item);
    } else {
      rows.push([item]);
...
function cardsFromDetectedGrids(
  ...
  const orderedGrids = sortSpatialItems(grids);
  const orderedIdentifiers = sortSpatialItems(identifiers);
...
export function shouldRereadBingoCell(value: number, index: number) {
  if (index % 5 === 0 && value >= 1 && value <= 9) return true;
...
export function identifierFamilyFromOcrText(text: string) {
  ...
  for (const match of normalized.matchAll(/juego\D{0,12}(\d{5,10})/gi)) add(match[1], 10);
...
async function recognizeMissingCells(
  ...
  const unresolvedRows = [...new Set(
    resolved
      .map((value, index) => ({ value, index }))
      .filter(({ value, index }) => index !== 12 && (!validNumberForCell(value, index) || value <= 0))
      .map(({ index }) => Math.floor(index / 5)),
  )];
  if (unresolvedRows.length) {
    ...
    for (const row of unresolvedRows) {
      ...
      const height = Math.max(1, rectangle.horizontalLines[row + 1] - top - 3);
      let decoded: number[] | null = null;
      for (const threshold of [110, 145, 180, 215]) {
        for (const maxChroma of [58, 256]) {
          ...
          if (
            decoded &&
            decoded.filter((v, col) => (row === 2 && col === 2) || validNumberForCell(v, row * 5 + col)).length >= 4
          ) {
            break;
          } else {
            decoded = null;
      ...
      if (decoded) {
        decoded.forEach((val, col) => {
          const idx = row * 5 + col;
          if (idx !== 12 && validNumberForCell(val, idx)) {
            resolved[idx] = val;
          }
        });
      }
...
function sabrositoTablaCells(x_left: number, y_top: number): RelativeNumberCell[] {
  return [
    cell(x_left + 0.070, y_top + 0.032, [1, 30], 0.08, 0.045, true),
    cell(x_left + 0.225, y_top + 0.032, [1, 75], 0.08, 0.045, true),
    cell(x_left + 0.380, y_top + 0.032, [1, 75], 0.08, 0.045, true),
    cell(x_left + 0.070, y_top + 0.150, [1, 30], 0.08, 0.045, true),
    cell(x_left + 0.380, y_top + 0.150, [1, 75], 0.08, 0.045, true),
  ];
}

const sabrositoSpecialCards: SpecialPageCard[] = [
  { suffix: 1, label: "Sabrosito", x: 0.25, y: 0.25, cells: sabrositoTablaCells(0.040, 0.155) },
  { suffix: 2, label: "Sabrosito", x: 0.75, y: 0.25, cells: sabrositoTablaCells(0.515, 0.155) },
  { suffix: 3, label: "Sabrosito", x: 0.25, y: 0.45, cells: sabrositoTablaCells(0.040, 0.355) },
  { suffix: 4, label: "Sabrosito", x: 0.75, y: 0.45, cells: sabrositoTablaCells(0.515, 0.355) },
  { suffix: 5, label: "Sabrosito", x: 0.25, y: 0.65, cells: sabrositoTablaCells(0.040, 0.555) },
  { suffix: 6, label: "Sabrosito", x: 0.75, y: 0.65, cells: sabrositoTablaCells(0.515, 0.555) },
  { suffix: 7, label: "Sabrosito", x: 0.25, y: 0.85, cells: sabrositoTablaCells(0.040, 0.755) },
  { suffix: 8, label: "Sabrosito", x: 0.75, y: 0.85, cells: sabrositoTablaCells(0.515, 0.755) },
];
...
export function specialPageLayoutFromOcrText(
  ...
  if (labelText.includes("SABROSITO") || labelText.includes("SABROSITOS")) {
...
async function recognizeSpecialPageCards(
  ...
  const layouts = specialLayout === "gordito"
    ? gorditoSpecialCards
    : specialLayout === "sabrosito"
      ? sabrositoSpecialCards
...
export function assignSequentialCardNumbers(cards: BingoCard[]) {
  ...
  // Detect duplicate non-SIN-ID numbers and mark subsequent occurrences as SIN-ID so gap filler can assign sequential values
  const seenNumbers = new Set<string>();
  for (let i = 0; i < resolved.length; i += 1) {
    const num = resolved[i].number;
    if (!num.startsWith("SIN-ID-")) {
      const lower = num.toLowerCase();
      if (seenNumbers.has(lower)) {
        resolved[i].number = `SIN-ID-${String(resolved[i].sourcePage).padStart(3, "0")}-${i + 1}`;
      } else {
        seenNumbers.add(lower);
      }
    }
  }
  ...
  // Final pass: ensure absolute uniqueness of all card numbers in batch
  const finalUsed = new Set<string>();
  for (let i = 0; i < resolved.length; i += 1) {
    let num = resolved[i].number;
    if (finalUsed.has(num.toLowerCase())) {
      let collision = 2;
      const base = num;
      while (finalUsed.has(`${base}-${collision}`.toLowerCase())) {
        collision += 1;
      }
      num = `${base}-${collision}`;
      resolved[i].number = num;
    }
    finalUsed.add(num.toLowerCase());
  }
